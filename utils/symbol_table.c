...
int insert_symbol(TDynamic_structure_buffer *struct_buff_tokens, index_t token_to_store, TDynamic_structure_buffer *struct_buff_nodes, index_t *index_to_root_node, TDynamic_buffer *b){
	...
	if (*index_to_root_node == ZERO_INDEX){
		*index_to_root_node = new_node_index;
		...
		create_node(*index_to_root_node, new_node_index, key, struct_buff_nodes, token_to_store);
	
	return RETURN_OK;
}

int iterate_through_tree(TDynamic_buffer *b, TDynamic_structure_buffer *struct_buff_tokens, char *str, TDynamic_structure_buffer *struct_buff_nodes, index_t actual_node_index, index_t *found_token_index){

	args_assert(struct_buff_tokens != NULL && str != NULL && b != NULL && struct_buff_nodes != NULL, INTERNAL_ERROR);
	...
	{
		v = iterate_through_tree(b, struct_buff_tokens, str, struct_buff_nodes, actual_node->right, found_token_index);
	}
	else if(key < actual_node->key)
	{
		v = iterate_through_tree(b, struct_buff_tokens, str, struct_buff_nodes, actual_node->left, found_token_index);
	}
	else if(key == actual_node->key)
	{
...
int find_symbol(TDynamic_structure_buffer *struct_buff_tokens, TDynamic_buffer *b, index_t index_to_string, TDynamic_structure_buffer *struct_buff_nodes, index_t index_to_root_node, index_t *found_token_index){
	...
	found = iterate_through_tree(b, struct_buff_tokens, str, struct_buff_nodes, index_to_root_node, found_token_index);
